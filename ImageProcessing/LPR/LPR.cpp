...
static int Internal_LPRProcess( pLPR_DATA ptrLPRData)
{
	...
	static int nFirst = 0;
	pCAMERA_DATA ptrCameraData=NULL;		/* 24bit 컬러 bmp */
	pIMAGE_DATA ptrGrayImageData = NULL;	/* GrayScale bmp */
	pIMAGE_DATA ptrChangeImageData = NULL;	/* mean, contrast, canny 처리 후 이미지 */
	pIMAGE_DATA ptrDestImageData = NULL;	/* 찾은 스트립 영역의 bmp */
	...
	/*===========================================================================
	[ 시트지 색상 HSI 및 복사 검출 단계 ]
	시트지 후보지에서 문자 이미지를 검출,판독
	============================================================================*/
	if ( nValidPlateCnt > 0 )
	{
