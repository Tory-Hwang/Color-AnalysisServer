...
LINE 출력 
============================================================================*/
#define PLATE_TITLE			15
#define CHAR_TITLE			( PLATE_TITLE + 50 )
#define CHAR_H_SIZE			20
#define CHAR_W_SIZE			100
#define LINE_INTERVAL		15
#define TAB_INTERVAL		10

void CWizardHelper::DrawRectLine( HDC hdc, int nImageH, CVRECT_t * ptrRect , int bDraw, int nExtX , int nExtY )
{
	int i ;
	...
	for( i = 0 ; i < MAX_LPR_AREA_CNT -1; i++ )
	{
		ptrTempRect = ( CVRECT_t*)&ptrRect[ i ];
		if ( ptrTempRect != NULL && ptrTempRect->nSeqNum > 0 )
		{
			//SetRect(&TextRect, nExtX, nImageH + PLATE_TITLE , nExtX + CHAR_W_SIZE *2 , nImageH + PLATE_TITLE + CHAR_H_SIZE );			
			//DrawText( hdc, L"<< 번호판 위치 정보 >>",-1, &TextRect, DT_WORDBREAK);

			nLeft	= ptrTempRect->nLeft + nExtX;
			nRight	= ptrTempRect->nRight + nExtX;
			nTop	= ptrTempRect->nTop + nExtY;
			nBottom = ptrTempRect->nBottom+ nExtY;
			...
			wsprintfW(tcText, L"[ IDX : %02d ], [ H:%d, W:%d ], [ T:%d, L:%d, R:%d, B: %d ]" ,i, nHeight, nWidth, nTop, nLeft, nRight, nBottom );
			
						
			nLeft	= nExtX + TAB_INTERVAL;
			nRight	= nLeft + CHAR_W_SIZE * 5 ;
			nTop	= nImageH + PLATE_TITLE + ( CHAR_H_SIZE * ( i + 1 ) );			
			nBottom = nTop + CHAR_H_SIZE;
...
void CWizardHelper::DrawCharRectLine( HDC hdc, int nImageH, int nCharCnt, CVRECT_t * ptrPlateRect ,  LPR_CHAR_t * ptrLPRChar ,int bDraw,  int nExtX, int nExtY)
{
	HPEN current_pen, old_pen;
	...
	{	
		...
		if ( ptrCharRect != NULL )
		{
			SetRect(&TextRect, nExtX, nImageH + CHAR_TITLE , nExtX + CHAR_W_SIZE *2 , nImageH + CHAR_TITLE + CHAR_H_SIZE );			
			DrawText( hdc, L"<< 문자 위치 정보 >> ",-1, &TextRect, DT_WORDBREAK);
			...
			nLeft	= nExtX + TAB_INTERVAL;
			nRight	= nLeft + CHAR_W_SIZE * 5 ;
			nTop	= nImageH + CHAR_TITLE + ( CHAR_H_SIZE * ( i + 1 ) );			
			nBottom = nTop + CHAR_H_SIZE;
...
void CWizardHelper::McsToWcs( char * Org, TCHAR * Dest, int OrgSize)
{
	USES_CONVERSION;	
	wcscpy_s( Dest, OrgSize + 1 , A2W( Org) );	
}

void CWizardHelper::WcsToMcs( TCHAR * Org, char * Dest, int OrgSize)
{
	USES_CONVERSION;
