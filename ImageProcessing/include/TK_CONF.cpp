...
int loadLPRConf( void )
{
...
#if 1
	g_lpr_conf.LPR_MIN_CHAR_H					= 20;
	g_lpr_conf.LPR_MIN_CHAR_W					= 20;	
#else
	g_lpr_conf.LPR_MIN_CHAR_H					= 9;
	g_lpr_conf.LPR_MIN_CHAR_W					= 2;	
#endif
	
	g_lpr_conf.LPR_UF_H							= UF_H_SIZE;
	g_lpr_conf.LPR_UF_W							= STRIP_W_SIZE;
	
	g_lpr_conf.LPR_MIN_CHAR_CNT					= 4;
	g_lpr_conf.LPR_MAX_CHAR_CNT					= 11;
	...
	g_lpr_conf.LPR_DBG_RUN_UF_DEL_H_LINE			= 1;
	...
	g_lpr_conf.LPR_DBG_SHOW_UF_VALID_BOX		= 1;

	g_lpr_conf.LPR_DBG_SHOW_BIG4_DIP			= 0;
	g_lpr_conf.LPR_DBG_SHOW_BIG4_FILTER			= 0;
...
int SetLPRConf( pLPR_CONF ptrLPRConf )
{
	...
	memset( szval, 0, sizeof( szval ));
	sprintf( szval, "%d", ptrLPRConf->LPR_DBG_RUN_UF_DEL_H_LINE );
	conf_session_set_value( ptrConf, SESSION_LPR, LPR_KEY_RUN_UF_GROUP_DEL_H_LINE, szval );
	...
	memset( szval, 0, sizeof( szval ));
	sprintf( szval, "%d", ptrLPRConf->LPR_DBG_SHOW_BIG4_FILTER );
	conf_session_set_value( ptrConf, SESSION_LPR, LPR_KEY_SHOW_BIG4_FILTER, szval );
...
int loadOCRConf( void)
{
	...
	if ( ptrVal )
	{
		...
		g_ocr_conf.OCR_CHAR_W = (short)(g_ocr_conf.OCR_CHAR_H * 0.65 );
	}
