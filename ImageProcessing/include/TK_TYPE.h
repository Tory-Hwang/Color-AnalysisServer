...
#define STRIP_W_SIZE						35  /* 스트립 개별 항목 W 사이즈 */
#define STRIP_H_SIZE						35	/* 스트립 개별 항목 H 사이즈 */
#define MAX_STRIP_CNT						11 /* 하나는 사용하지 않아 총 10개만 이용 */

#define UF_H_SIZE							2 /* 불균등 체크 X 간격 */
#define UF_W_SIZE							STRIP_W_SIZE /* 불균등 체크 Y 간격 */

#define MAX_LPR_CHAR_CNT					MAX_STRIP_CNT -1	/* 유효 시트지 개수 */
#define MAX_LPR_PLATE_CANDIDATE_CNT			MAX_STRIP_CNT
#define MAX_LPR_AREA_CNT					MAX_STRIP_CNT	/* LPR 인식 영역 수 */

#define STRIP_CENTER						(MAX_STRIP_CNT/2) /* 스트립 항목 중간 계수 */

#define MAX_PLATE_RECT_IMAGE_CNT			MAX_STRIP_CNT+1  /* 최대 스트립 항목 영역 개수 */
#define MAX_CHAR_LABEL_CNT					MAX_STRIP_CNT+1

#define STRIP_BOTTOM_POS					300	/* 스트립지 최대 Bottom 위치 */
#define STRIP_RIGHT_POS						55	/* 스트립지 최소 Right 위치 */

#define MAX_STRIP_NAME_SIZE					30	/* 스트립지 항목 이름 사이즈 */
#define FIX_CELL_SIZE						1	/* 스트립지 영역 테두리 영역 사이즈 */
#define MAX_STRIP_LEVEL_CNT					6 /* STRIP 색상 LEVEL 단계 */
#define HSI_SIZE 5
...
#define TK_DIP_SOBEL5_MSG						0x1028
...
{	
	...
	BYTE LPR_DBG_RUN_UF_DEL_H_LINE;		/* Uniformity 수직 영영 재 설정 */
	...
	BYTE LPR_DBG_SHOW_BIG4_FILTER;			/* 4자리 숫자 DIP FILTER 처리 후 보기  */
...
typedef struct STRIN_NAME_
{
	TCHAR NAME[ MAX_STRIP_NAME_SIZE ];
}STRIN_NAME_t;

typedef struct STRIP_HSI_
{
	float fh;
	float fs;
	float fi;
}STRIP_HSI_t, *pSTRIP_HSI;

typedef struct STRIP_LEVEL_
{
	STRIP_HSI_t hsi[ MAX_STRIP_LEVEL_CNT];
}STRIP_LEVEL_t, *pSTRIP_LEVEL;
