...
int HLP_CopyImageDataWithRect24( IMAGE_DATA_t * ptrDestImageData , IMAGE_DATA_t * ptrSrcImageData, CVRECT_t *ptrRect )
{
	int nLineByte=0;
	int nColors=0;
	int i;
	...
	for (i=ptrRect->nTop; i< ptrRect->nBottom; i++)
	{
		if ( i < ptrSrcImageData->nH )
		{	
			memcpy( *( ptrDestImageData->ptrSrcImage2D + nTopPos ) + nLeftPos , *( ptrSrcImageData->ptrSrcImage2D + i )+ ptrRect->nLeft * 3, nOneLineByte );			
		}
		nTopPos++;
	}
